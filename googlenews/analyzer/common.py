...
def analyze_eltiempo(response:HtmlResponse, **kwargs):
...
@ANALYZER_REGISTRY.register()
def analyze_caracol(response:HtmlResponse, **kwargs):
    """
    ...
    title = parse_html_to_text_by_xpath(response, '/html/body/main/div[2]/article/div/header/h1')
    alternative_title =  parse_html_to_text_by_xpath(response, '/html/body/main/div[2]/article/div/header/h2')
    content_list = response.xpath('/html/body/main/div[2]/article/div/div[2]/div[3]/div[2]/div[1]/p')
    ...
    video_url = []
    videodata = response.xpath('/html/body/main/div[2]/article/div/div[1]/script[2]//text()').get()
    if videodata:
        videodata = json.loads(videodata.strip())
        video_url.append(videodata['contentURL'])
    image_urls = []
    imagedata = response.xpath('/html/body/main/div[2]/article/div//figure//img/@data-src').getall()
    if imagedata:
        image_urls.extend(['https:'+u for u in imagedata])
    video_foto_data = response.xpath('/html/body/main/div[2]/article/div/div[1]/div/@data-fotonoticia').getall()
    if video_foto_data:
